...
JRE는 JVM이 자바 프로그램을 동작시킬 때 필요한 라이브러리 파일들과 기타 파일들을 가지고 있어, JRE는 JVM의 실행환경을 구현했다고 말할 수 있습니다.
자바 프로그램을 실행시키기 위해서는 JRE를 반드시 설치해야 하지만 자바 프로그래밍 도구(javac, java등)는 포함되어있지 않기 때문에 프로그래밍을 위해서는 JDK를 설치해줘야 합니다.


#### JDK : 자바 프로그램밍시 필요한 컴파일러를 포함한 개발 도구
JDK는 개발을 위해 필요한 도구들을 포함하며, JDK를 설치하면 JRE도 함께 설치가 됩니다. 크기는 **JVM < JRE < JDK**순으로 진행됩니다.

### 개선할점 및 느낀점
처음으로 Git markdown과 함께 글을 작성하다보니 낯설고 술술 풀리지 않았으나 1주차 ~ 18주차까지 진행하면 금방 익힐 수 있을거라 생각합니다. java는 사용하고 있지만 정확한 개념없이 사용하는 경우가 많았는데 글을 정리하며 작성하다보니 새로 알게된 개념이 많아 유익한 시간이었습니다.

아직 많이 부족하지만 천천히 반복학습하며 공부해보도록 하겠습니다.
